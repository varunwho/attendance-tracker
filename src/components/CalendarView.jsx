...
export default function CalendarView({ attendanceMap, holidayDates, markDay }) {
  ...
  function cancelSelection() { setSelected(new Set()) }
  function prevMonth() { setViewDate(new Date(year, month - 1, 1)); setSelected(new Set()) }
  function nextMonth() { setViewDate(new Date(year, month + 1, 1)); setSelected(new Set()) }
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    // Determine bg + text color — all class names are complete literals for Tailwind v4 detection
    ...
    const ring = isSelected
      ? (isHoliday ? 'ring-2 ring-gray-400 opacity-60' : 'ring-2 ring-blue-300')
      : isToday && !isHoliday
    ...
    <div className="flex flex-col gap-4">
      ...
      <p className="text-gray-500 dark:text-gray-500 text-xs text-center">
        {isDragging
          ...
          : hasSelection
          ? `${selected.size} day${selected.size !== 1 ? 's' : ''} selected — choose action below`
      ...
      <div className="flex flex-wrap gap-3 text-xs text-gray-500 dark:text-gray-400">
        ...
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-gray-200 dark:bg-gray-700 inline-block" /> Holiday / Weekend</span>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 flex flex-col gap-3 shadow-sm">
        ...
        <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-1.5">
          <div
            className={`h-1.5 rounded-full transition-all ${onTrack ? 'bg-green-500' : 'bg-red-500'}`}
      ...
      >
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-3 flex gap-2 shadow-xl mb-20" style={{ marginBottom: 'calc(5rem + env(safe-area-inset-bottom, 0px))' }}>
          ...
          <button onClick={() => applyMode('clear')}   className="flex-1 bg-gray-600 hover:bg-gray-500 text-white rounded-xl py-3 text-sm font-semibold transition-colors">Clear</button>
          <button onClick={cancelSelection}            className="bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-xl px-3 text-sm transition-colors">✕</button>
        </div>
      </div>
    </div>
  )
